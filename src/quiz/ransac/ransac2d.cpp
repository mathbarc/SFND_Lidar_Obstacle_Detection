...
std::unordered_set<int> Ransac(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations,
    float distanceTol
)
{
...
int main()
{
    ...
    for (int index = 0; index < cloud->points.size(); index++)
        {
            ...
            if (inliers.count(index)) cloudInliers->points.push_back(point);
            else cloudOutliers->points.push_back(point);
        }
    ...
    else { renderPointCloud(viewer, cloud, "data"); }
