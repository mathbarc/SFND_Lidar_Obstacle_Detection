...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
    ...
    viewer->addCube(
        window.x_min, window.x_max, window.y_min, window.y_max, 0, 0, 1, 1, 1,
        "window"
    );
    return viewer;
}
...
void render2DTree(
    Node *node, pcl::visualization::PCLVisualizer::Ptr &viewer, Box window,
    ...
    if (node != NULL)
        {
            ...
            render2DTree(
                node->right, viewer, upperWindow, iteration, depth + 1
            );
        }
}
    ...
    for (std::vector<int> cluster : clusters)
        {
            ...
            renderPointCloud(
                viewer, clusterCloud, "cluster" + std::to_string(clusterId),
                colors[clusterId % 3]
            );
            ++clusterId;
        }
