...
void renderHighway(pcl::visualization::PCLVisualizer::Ptr &viewer)
{
    ...
    viewer->addCube(
        -roadLength / 2, roadLength / 2, -roadWidth / 2, roadWidth / 2,
    ...
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, .2, .2, .2, "highwayPavement"
    );
    viewer->setShapeRenderingProperties(
...
)
{
    ...
    for (pcl::PointXYZ point : cloud->points)
        {
            viewer->addLine(
                pcl::PointXYZ(origin.x, origin.y, origin.z), point, 1, 0, 0,
...
)
{
    ...
    if (color.r == -1)
    ...
    else
        {
            ...
            viewer->setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_COLOR, color.r, color.g,
                color.b, name
            );
        }
...
void renderBox(
    pcl::visualization::PCLVisualizer::Ptr &viewer, Box box, int id,
...
)
{
    ...
    std::string cube = "box" + std::to_string(id);
    viewer->addCube(
        box.bboxTransform, box.bboxQuaternion, box.cube_length, box.cube_width,
