...
struct Ray
{
        ...
        // horizontalAngle: the angle of direction the ray travels on the xy
        // plane verticalAngle: the angle of direction between xy plane and ray
        // 				  for example 0 radians is along xy
        // plane and pi/2 radians is stright up resoultion: the magnitude of the
        // ray's step, used for ray casting, the smaller the more accurate but
        // the more expensive
        ...
        void rayCast(
            const std::vector<Car> &cars, double minDistance,
            ...
            while (!collision && castDistance < maxDistance)
                {
                    ...
                    collision =
                        (castPosition.z <= castPosition.x * tan(slopeAngle));
                    ...
                    if (!collision && castDistance < maxDistance)
                        {
                            for (Car car : cars)
                                {
                                    collision |=
                                        car.checkCollision(castPosition);
                                    if (collision) break;
                                }
                        }
                }
...
struct Lidar
{
        ...
        // TODO:: increase number of layers to 8 to get higher resoultion
        ...
        Lidar(
            std::vector<Car> setCars, double setGroundSlope, int numLayers = 3,
            double horizontalAngleInc = pi / 6, double minDistance = 0,
            double sderr = 0.0
        )
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr scan()
        {
            ...
            for (Ray ray : rays)
                ray.rayCast(
                    cars, minDistance, maxDistance, cloud, groundSlope, sderr
                );
            ...
            cout << "ray casting took " << elapsedTime.count()
                 << " milliseconds" << endl;
            cloud->width = cloud->points.size();
            cloud->height =
                1; // one dimensional unorganized point cloud dataset
            return cloud;
        }
};
